...
import type { Opportunity, SamOpportunityRaw, FilterState, NoticeType } from '../types'

const SAM_BASE = 'https://api.sam.gov/opportunities/v2/search'
const API_KEY = import.meta.env.VITE_SAM_GOV_API_KEY as string

// ─── Raw → App shape mapper ───────────────────────────────────────────────────
function mapOpportunity(raw: SamOpportunityRaw): Opportunity {
  return {
    id: raw.noticeId,
    title: raw.title,
    solicitationNumber: raw.solicitationNumber ?? '—',
    agency: raw.organizationHierarchy?.l1Name ?? raw.fullParentPathName?.split('.')[0] ?? 'Unknown Agency',
    subAgency: raw.organizationHierarchy?.l2Name,
    noticeType: (raw.type as NoticeType) ?? 'Other',
...
export async function searchOpportunities(
  ...
  // When no real API key — return demo data
  ...
  if (filters.deadlineDays) {
    ...
    const to = new Date(Date.now() + filters.deadlineDays * 86400000).toISOString().split('T')[0]
    params.set('rdlfrom', from)
    params.set('rdlto', to)
  }
  ...
  return {
    data: (json.opportunitiesData ?? []).map(mapOpportunity),
    total: json.totalRecords ?? 0,
  }
}

// ─── Demo / Seed data (used when no API key is set) ───────────────────────────
export function getDemoData(_filters?: Partial<FilterState>): { data: Opportunity[]; total: number } {
  const DEMO: Opportunity[] = [
    {
      ...
      status: 'active',
      description: 'The Department of State requires AI-powered data processing services including NLP, document classification, and automated report generation for diplomatic communications.',
      matchScore: 89,
    ...
    },
    {
      ...
      status: 'active',
      description: 'Migration of legacy on-prem workloads to AWS GovCloud. Contractor must hold FedRAMP authorization and have demonstrated cloud migration experience.',
      matchScore: 72,
    ...
    },
    {
      ...
      setAside: 'SDVOSB',
      status: 'active',
      description: 'DHS/CISA seeks qualified vendors to provide comprehensive cybersecurity risk assessments, red-team exercises, and penetration testing for critical infrastructure systems.',
      matchScore: 55,
    ...
    },
    {
      ...
      status: 'active',
      description: 'Full-stack development of a modernized benefits administration portal using React, Node.js, and PostgreSQL. Accessibility (WCAG 2.1 AA) compliance required.',
      matchScore: 91,
    ...
    },
    {
      ...
      setAside: 'HUBZone',
      status: 'active',
      description: 'Tier 1-3 IT helpdesk support, hardware maintenance, and network troubleshooting across 12 military installations. Secret clearance required for key personnel.',
      matchScore: 34,
    ...
    },
    {
      ...
      status: 'active',
      description: 'Design, train and deploy ML models for cancer biomarker detection. Requires experience with Python, TensorFlow/PyTorch, and HIPAA-compliant cloud environments.',
      matchScore: 78,
